...
class Welcome extends CI_Controller {
    ...
    //functions for client signature (in, update, out, on, off)
    public function check_user_for_sing_in() {
        ...
        if($data_insta['success']==true){
            ...
            if($N==0){ //si no existe em dumbus
                ...
                $client=$client[0];
                $user=$this->user_model->get_user_from_client_id($client['user_id'])[0];
   ...
   /* public function client_list_active_profiles(){
    ...
    public function message(){
        $response['success']=true;
        $response['message']='Mensagem enviada, agradecemos seu contato ...';
    }
    ...
    public function get_day_of_payment(){
        $this->load->model('class/dumbu_system_config');
        /*TODO: si oferta promocional TIPO 1 adicionar N dias gratis*/
